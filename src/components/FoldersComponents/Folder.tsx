      ...
      <div className="mt-2 mb-6 grid">
        <div className="grid grid-cols-6 border-b pb-1 px-2">
          ...
          <label className="flex items-center cursor-pointer relative" htmlFor={"check"}>
                    <input type="checkbox" onChange={(e)=>{if(e.target.checked){setCheckedFolders(currentView?currentView.subFolders.map(folder=>folder.id):folders.map(folder=>folder.id))}else{setCheckedFolders([])}}} checked={currentView?checkedFolders.length===currentView.subFolders.length&&checkedFolders.length>0:checkedFolders.length===folders.length&&checkedFolders.length>0}
