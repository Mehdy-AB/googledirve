...
import RolesInTable from "@/components/admin/roles/RolesInTable";
...
const Page=()=>{
  const sidebarOpen =true;
    ...
    const getRoles = () => {
      axiosClient
          ...
          .then((response) => {
            setRoles(response.data); // Display subfolders and files of the clicked folder
          })
          .catch((error) => setAlerts((prv)=>[...prv,{type:2,message:'error in getting roles'}]));
      };
  ...
  useEffect(getRoles,[]);
    ...
    const addRuleLine=(ruleLine)=>{
        setRoles([...Roles,{...ruleLine,activate:false,createdAt:new Date()}]);

        ///save it to back end 
          axiosClient
          .post("/backReq/admin/Roles", {data:ruleLine})
          .then((response) => {
            setRoles(response.data); // Display subfolders and files of the clicked folder
  ...
  const duplicateItems = (indexes: number[]) => {
    setRoles((prevRoles) => {
      const duplicatedItems = indexes.map((idx) => ({
        ...prevRoles[idx],
        name: `${prevRoles[idx].name} (Copy)`, // Optional: append "Copy" to distinguish duplicates
        createdAt: new Date().toISOString(), // Set new creation date for the duplicate
      }));
      return [...prevRoles, ...duplicatedItems];
    });
  };

  // Function to update a specific index with new data
    const updateRoles = (index: number, newData: data) => {
        setRoles((prev) => {
          ...
          updatedRoles[index] = newData; // Replace the item at the given index
          return updatedRoles;
        });
      };
  ...
  const exportToExcel = (indices: number[]) => {
    const selectedData =
      indices.includes(-1) ? Roles : indices.map((index) => Roles[index]);
 ...
 const importFromExcel = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      // Update state
      setRoles((prev) => [...prev, ...formattedData]);
    };
  ...
  const importFromCsv = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return(
        <div className="h-full" >
             {showForm && (edit?
             <CreateRoleForm onClose={setShowForm} addRole={addRuleLine} sidebarOpen={sidebarOpen} edit={{Role:edit.data,update:updateRoles,index:edit.index}}/>:
             <CreateRoleForm onClose={setShowForm} addRole={addRuleLine} sidebarOpen={sidebarOpen} />)
            } 

             <div className="p-10 mt-10 grid">
                <div className="grid grid-cols-2">
                    <span className="text-xl font-bold"> Roless : </span>
                    <div className="relative justify-end flex "> 

                    <button onClick={()=>{setDropDownRoles(!dropDownRoles)}}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="rounded-full mr-2 bg-white cursor-pointer size-6">
                    ...
                    <input id="Import-csv"
                                            ...
                                            />
                    {dropDownRoles&& <DropDown setIsShow={setDropDownRoles}>
                        <div className="z-30 absolute bg-white top-7 shadow-xl border py-2 right-44 ">
                            <button onClick={()=>setSelectedRoles(Roles.map((_, index) => index))} className=" w-full pl-4 p-1 flex justify-start mt-2 items-center hover:bg-gray-200">
                    ...
                    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" onClick={()=>duplicateItems(selectedRoles)} className={`${selectedRoles.length === 0?"text-slate-300 ":"hover:bg-white cursor-pointer"} size-5 mr-1 rounded-full`}>
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" onClick={()=>deleteItems(selectedRoles)} className={`${selectedRoles.length === 0?"text-slate-300 ":"hover:bg-white cursor-pointer"} size-5 mr-1 rounded-full`}>
                                        <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                        </svg>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className={`${selectedRoles.length === 0?"text-slate-300 ":"hover:bg-white cursor-pointer"} size-5 mr-1 rounded-full`}>
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M18 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0ZM3 19.235v-.11a6.375 6.375 0 0 1 12.75 0v.109A12.318 12.318 0 0 1 9.374 21c-2.331 0-4.512-.645-6.374-1.766Z" />
                                                    </svg>
                    <svg viewBox="0 0 24 24" fill="none" onClick={()=>exportToCsv(selectedRoles)} className={`${selectedRoles.length === 0?"text-slate-300 ":"hover:bg-white cursor-pointer"} size-6 mr-1 rounded-full`} xmlns="http://www.w3.org/2000/svg">
                                        <path d="M12 5L11.6464 4.64645L12 4.29289L12.3536 4.64645L12 5ZM12.5 14C12.5 14.2761 12.2761 14.5 12 14.5C11.7239 14.5 11.5 14.2761 11.5 14L12.5 14ZM6.64645 9.64645L11.6464 4.64645L12.3536 5.35355L7.35355 10.3536L6.64645 9.64645ZM12.3536 4.64645L17.3536 9.64645L16.6464 10.3536L11.6464 5.35355L12.3536 4.64645ZM12.5 5L12.5 14L11.5 14L11.5 5L12.5 5Z" fill="currentColor"/>
                                        <path d="M5 16L5 17C5 18.1046 5.89543 19 7 19L17 19C18.1046 19 19 18.1046 19 17V16" stroke="currentColor"/>
                                        </svg>
                    </div>
             ...
             <div className="my-4">
              <form action="/search" className="max-w-[480px] mx-auto px-4">
                  <div className="relative">
                      <input type="text" name="q" className="w-full focus:outline-none focus:ring-1 border shadow py-2 p-4 rounded-full " placeholder="search"/>
                      <button type="submit">
                          <svg className="text-secondColor h-5 w-5 absolute top-3 right-3 fill-current "
             ...
             <div className="shadow-lg rounded-lg overflow mx-4 md:mx-10">
                ...
                <div className=" items-center border-2 border-dashed rounded-lg border-gray-300">
                    ...
                    <span className="text-secondColor mt-16 flex justify-center font-semibold ">Pas encore de Roles</span>
                ...
                <table className="w-full ">
                        <thead>
                            <tr className="bg-gray-100 text-xs">
                                ...
                                <th className="w-1/4 py-2 px-6 text-left text-gray-600 font-semibold uppercase">Users</th>
                                <th className="w-1/4 py-2 px-6 text-left text-gray-600 font-semibold uppercase">Groups</th>
                            </tr>
                        </thead>
                    <tbody className="bg-white">
                    {Roles.map((role, index) =>
                            <RolesInTable key={index} setSelectedRoles={setSelectedRoles} selectedRoles={selectedRoles} index={index} role={role} duplicateItems={duplicateItems} deleteItems={deleteItems} setShowForm={setShowForm} setEdit={setEdit} Roles={Roles} exportToCsv={exportToCsv} exportToExcel={exportToExcel} />)
                            }
                    </tbody>
                </table>}
            </div>
